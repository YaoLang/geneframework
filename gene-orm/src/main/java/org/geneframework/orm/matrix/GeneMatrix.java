...
public class GeneMatrix implements Matrix {
	...
	private java.util.Vector<GeneMatrixListenr> geneMatrixListenrs = new java.util.Vector<>();
	...
	public void registGeneMatrixListener(GeneMatrixListenr geneMatrixListenr){
		if(!geneMatrixListenrs.contains(geneMatrixListenr))
			geneMatrixListenrs.add(geneMatrixListenr);
	}

	public void removeGeneMatrixListener(GeneMatrixListenr geneMatrixListenr){
		if(geneMatrixListenrs.contains(geneMatrixListenr))
			geneMatrixListenrs.remove(geneMatrixListenr);
	}

	public void notifyGeneMatrixListener(){
		for (int i = 0; i < geneMatrixListenrs.size(); i++) {
			geneMatrixListenrs.get(i).setGeneMatrix(this);
		}
	}
}
